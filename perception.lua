...
function init()
  ...
  --smpl
    ...
    softcut.play(rec_voice, play)
    softcut.fade_time(rec_voice, 0.5)
    
    softcut.level_input_cut(1,1,1.0)
    -- softcut.level_input_cut(2,1,1.0)
    softcut.rec_level(rec_voice,1)
    softcut.pre_level(rec_voice,0.5)
    softcut.rec(rec_voice,rec)
...
function load_audio(path)
    if path ~= "cancel" and path ~= "" then
      ...
      if ch > 0 and len > 0 then
        ...
        softcut.buffer_read_stereo(path, 0, 1, -1, 1, 2, 0, 1)
